...
def optimize(radius, density, escape_velocity, surface_temperature):
    ...
    def f1(x):
        ...
        objective = -score + equality_penalty(1 - (x[0] + x[1]), tolerance) +inequality_penalty(-x[0],error) + inequality_penalty(x[0]-1,error)+inequality_penalty(-x[1],error)+inequality_penalty(x[1]-1,error)
        return objective
